...
def train(args, train_dataset, model, tokenizer):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            loss = outputs[0]
            KL = outputs[-1]
            if global_step >= args.kl_start_step and (
                    args.att_type == 'soft_weibull' or args.att_type == 'soft_lognormal') and args.att_kl != 0:

                step_diff = global_step - args.kl_start_step
                step_diff = step_diff * args.kl_anneal_rate
                loss = loss + args.att_kl * np.exp(step_diff) / (1 + np.exp(step_diff)) * KL
...
def main():
    ...
    parser.add_argument('--kl_start_step', type=float, default=0.0,
                        help='step that starts to add kl term.')
    ...
    parser.add_argument('--att_contextual_se', type=int, default=1,
    ...
    parser.add_argument('--label_noise', type=float, default=0.0,
                        help='percentage of labels with noise added.')
    ...
    va_args_dict = {"att_type": args.att_type,
                    "k_weibull": args.k_weibull,
                    "att_prior_type": args.att_prior_type,
                    "alpha_gamma": args.alpha_gamma,
                    "beta_gamma": args.beta_gamma,
                    "sigma_normal_prior": args.sigma_normal_prior,
                    "sigma_normal_posterior": args.sigma_normal_posterior,
                    "att_contextual_se": args.att_contextual_se,
                    "att_se_hid_size": args.att_se_hid_size,
                    "att_se_nonlinear": args.att_se_nonlinear,
